...
{
    //different state variables wich determines what is the player doing
    ...
    [Header("Componentes de inspector")]
    ...
    void Update()
    {
        currentState.Update(); //run update method of current state
        if (!isSliding)
            Flip(); //if player is sliding do not flip
        AnimationHandler(); //this just updates general animation variables
    }
    ...
    private void FixedUpdate()
    {
        ...
        CheckForWall(); //same for walls (same layer but search horizontally)
    }
    ...
    private void OnMove(InputValue value) //reads move inputs on input detection
    {
    ...
    public void SetColliderHeight(float height, Vector2 offset) //cambia hitbox a los valores dados
    {
        boxCollider.size = new Vector2 (boxCollider.size.x, height);
        boxCollider.offset = offset;
    }
    private void Flip() //girar jugador horizontalmente
    {
    ...
    private void AnimationHandler() //controlador de animaciones
    {
        anim.SetBool("isGrounded", isGrounded);
        anim.SetFloat("yVelocity", rigidbody2.linearVelocityY);
    }
    private void CheckForGround() //verificar si esta tocando suelo
    {
    ...
    private void OnDrawGizmosSelected() //dibujar gizmos para vizualisar raycasts
    {
